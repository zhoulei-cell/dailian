	...
	<view class="release-order">
		<view class="wrapper">
			...
			<view class="list-group">
				...
				<view class="list-item">
					<view class="item-title">订单价格(价格不能修改)</view>
					<view class="item-text d-flex ai-center jc-between">
						<input class="text flex-1 gray" v-model="info.price" placeholder="请在此输入" placeholder-class="placeholder" :disabled="true"/>
			...
			<view class="list-group">
				...
				<view class="list-item">
					<view class="item-box d-flex ai-center jc-between">
						<view class="left">游戏账号(账号不能修改)</view>
						<input class="gray" type="text" v-model="info.game_account" placeholder="请填写正确的游戏账号" :disabled="true" placeholder-class="placeholder"/>
					</view>
				</view>
				<view class="list-item">
					<view class="item-box d-flex ai-center jc-between">
			...
			getorderdetail(id){
				...
				this.$http.httpTokenRequest(opts, params).then(res => {
					if (res.data.code == 200) {
						...
						this.info.area = this.info.platform.name + "*" + this.info.game_area.name
						this.multiIndex = [this.info.platform.id, this.info.game_area.id] 
